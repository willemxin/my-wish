window.onload = function() {
    ...
    (function() {
        ...
        oBtn.onclick = function() {
            ajax("wish.php?act=add&username=" + oUserName.value + "&content=" + oCon.value,
            function(str) {
                var json = eval("(" + str + ")");
                if (json.error) {
                    alert(json.msg)
        ...
        function(str) {
            ...
            if (json.error) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    oDl.appendChild(oDd);
                    oBox.appendChild(oDl); (function(index) {
                        oDd.children[1].onclick = function() {
                            if (confirm("确认删除此心愿么?")) {
                                ajax("wish.php?act=delete&id=" + arr[index].id,
                                function(str) {
                                    var json = eval("(" + str + ")");
                                    if (json.error) {
                                        alert(json.msg)
